...
# New: pandas DataFrames cached in memory
_DATA_CTX = None

class DataContext:
    """
    Hold all denormalised CSVs as pandas DataFrames, loaded once.

    Keys / relationships follow the production README:
    - partner.csv: partner_id
    - partner_role.csv: partner_id, entity_id, entity_type
    - business_rel.csv: br_id
    - br_to_account.csv: br_id, account_id
    - account.csv: account_id
    - transactions.csv: Account ID
    - partner_country.csv: partner_id, country_type
    """
    def __init__(self) -> None:
        self.partner = self._load("partner.csv").set_index("partner_id")

        self.partner_role = self._load("partner_role.csv")
        self.business_rel = self._load("business_rel.csv").set_index("br_id")
        self.br_to_account = self._load("br_to_account.csv")

        self.account = self._load("account.csv").set_index("account_id")
        self.transactions = self._load("transactions.csv")

        self.partner_country = self._load("partner_country.csv")

    @staticmethod
    def _load(filename: str) -> pd.DataFrame:
        path = os.path.join(DATA_DIR, filename)
        # All as string to avoid dtype headaches; parse dates manually when needed
        return pd.read_csv(path, dtype=str)


def _data() -> DataContext:
    """
    Lazy singleton accessor for all DataFrames.
    """
    global _DATA_CTX
...
'''def _load_csv(filename: str) -> list[dict]:
    ...
    return rows'''
def _load_csv(filename: str) -> list[dict]:
    """Legacy: list-of-dict reader with caching."""
    if filename in _CSV_CACHE:
        return _CSV_CACHE[filename]
...
def _get_partner_record(partner_id: str) -> dict | None:
    ctx = _data()
    try:
        row = ctx.partner.loc[partner_id]
    except KeyError:
        return None
...
def _get_partner_brs(partner_id: str) -> set[str]:
    ...
    """
    ctx = _data()
    roles = ctx.partner_role

    mask = (roles["partner_id"] == partner_id) & (roles["entity_type"] == "BR")
    br_ids = roles.loc[mask, "entity_id"].dropna().unique()
...
def _get_br_accounts(br_ids: set[str]) -> set[str]:
    ...
    ctx = _data()
    links = ctx.br_to_account

    mask = links["br_id"].isin(br_ids)
    account_ids = links.loc[mask, "account_id"].dropna().unique()
...
def _get_accounts(account_ids: set[str]) -> list[dict]:
    ...
    # use .reindex to avoid KeyError if some IDs are missing
    sub = acc.reindex(list(account_ids)).dropna(how="all")

    # preserve existing list-of-dict contract
    return sub.reset_index().to_dict(orient="records")
...
def _get_transactions_for_accounts(account_ids: set[str]) -> list[dict]:
    ...
    mask = tx["Account ID"].isin(account_ids)
    sub = tx.loc[mask]
...
def resolve_customer(query: str) -> str:
    ...
    ctx = _data()
    partners = ctx.partner.reset_index()  # bring partner_id back as a column

    matches: list[dict] = []

    # Exact partner_id match
    exact = partners.loc[partners["partner_id"].str.lower() == q]
    for _, row in exact.iterrows():
        matches.append({
            "customer_id": row["partner_id"],
    ...
    # Partial name / phone (if we still need more)
    if len(matches) < 5 and q:
        # Normalised columns
        name_col = partners["partner_name"].fillna("").str.lower()
        phone_col = partners["partner_phone_number"].fillna("").str.replace(" ", "").str.lower()

        mask = name_col.str.contains(q) | phone_col.str.contains(q)
        fuzzy = partners.loc[mask]
...
def get_customer_profile(customer_id: str) -> str:
    ...
    for row in country_rows:
        ...
        # Country: prefer domicile row from partner_country.csv
    ctx = _data()
    pc = ctx.partner_country
    mask = (
        (pc["partner_id"] == customer_id)
        & (pc["country_type"] == "domicile")
        & (pc["partner_country_status_code"] == "1")
    )
    match = pc.loc[mask]

    country = None
    if not match.empty:
        # If multiple, just take the first active domicile row
        country = match.iloc[0]["country_name"]
